...
from src.utils.audioclip.models.clip.model import CLIP
from src.utils.audioclip.models.clip.clip import tokenize
from src.utils.audioclip.models.esresnet.fbsp import ESResNeXtFBSP
    ...
    def __init__(self,
        ...
        if isinstance(self.pretrained, str):
            ...
            self.load_state_dict(torch.load(
                str(xeno_audioclip_models_dir / 'CLIP.pt'),
                map_location='cpu'
            ), strict=False)
            print('Image & Text weights loaded')
            try:
                self.audio.load_state_dict(torch.load(
                    str(xeno_audioclip_models_dir / 'ESRNXFBSP.pt'),
                    map_location='cpu'
                ), strict=False)
